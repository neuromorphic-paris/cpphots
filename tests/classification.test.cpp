...
int main() {
    ...
    cpphots::StandardClassifier classifier1({"cl", "di", "he", "sp"});

    classifier1.setClassFeatures("cl", { 45,  46,  28, 372,  82,  65, 222, 217, 351, 262, 291, 107, 123,  73, 169, 112});
    classifier1.setClassFeatures("di", { 41,  48,  71, 288,  93, 199, 145, 325, 159, 340, 228,  39, 452, 157, 205, 325});
    classifier1.setClassFeatures("he", { 32,  63,  43, 262, 150, 118,  71, 102, 583, 103, 310,  35, 444, 163,  62, 467});
    classifier1.setClassFeatures("sp", { 92,  53,  76, 205, 114, 610,  83, 388, 189, 649, 446,  32, 253, 234, 115, 520});

    cpphots::NormalizedClassifier classifier2(&classifier1);
    cpphots::BhattacharyyaClassifier classifier3(&classifier1);

    std::vector<std::string> realclasses = {"cl", "di", "di", "sp", "di", "he", "he", "he", "sp", "di", "he", "di", "he", "he", "he", "cl", "cl", "cl", "he", "sp", "di", "cl", "cl", "he", "di", "cl", "sp", "sp", "sp", "cl", "sp", "di", "di", "di", "cl", "he", "di", "di", "cl", "he", "sp", "di", "cl", "cl", "sp", "cl", "he", "he", "cl", "sp", "cl", "di", "sp", "he", "sp", "cl", "cl", "sp", "di", "di", "sp", "he", "di", "he", "sp", "sp", "sp", "he"};
    ...
    for (unsigned int i = 0; i < 68; i++) {
        if (classifier1.classifyName(computed_features[i]) == realclasses[i])
            acc1 += 1.0;
        if (classifier2.classifyName(computed_features[i]) == realclasses[i])
            acc2 += 1.0;
        if (classifier3.classifyName(computed_features[i]) == realclasses[i])
            acc3 += 1.0;
    }
    ...
    std::cout << "acc1 = " << acc1 << ", acc2 = " << acc2 << ", acc3 = " << acc3 << std::endl;

    if (std::abs(acc1 - 0.734375) >= 0.01)
        return 1;
    if (std::abs(acc2 - 0.8125) >= 0.01)
        return 1;
    if (std::abs(acc3 - 0.78125) >= 0.01)
        return 1;
